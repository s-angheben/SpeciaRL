import logging
import pandas as pd
from pathlib import Path
from typing import Any, Dict, List

import datasets

from .abstract import LocalDatasetBuilder
from ..utils import download_file, extract_tar

logger = logging.getLogger(__name__)


class Flowers102Loader(LocalDatasetBuilder):
    @property
    def cache_name(self) -> str:
        return "flowers102"

    @property
    def style(self) -> str:
        return "image_classification"

    @property
    def ability(self) -> str:
        return "accuracy"

    @classmethod
    def get_available_splits(cls) -> List[str]:
        return ["train", "test", "val"]

    def _build(self, output_path: Path):
        raw_data_dir = self.cache_dir / "raw" / self.cache_name
        data_url = "https://www.robots.ox.ac.uk/~vgg/data/flowers/102/102flowers.tgz"
        tar_path = raw_data_dir / Path(data_url).name
        images_base_path = raw_data_dir / "jpg"

        if not images_base_path.exists():
            download_file(data_url, tar_path)
            extract_tar(tar_path, raw_data_dir, expected_content_name="jpg")

        asset_dir = Path("configs/dataset_assets/flowers102")
        labels_fp = asset_dir / "labels.csv"
        metadata_fp = asset_dir / "metadata.csv"
        split_fp = asset_dir / "split_coop.csv"

        if not all([labels_fp.exists(), metadata_fp.exists(), split_fp.exists()]):
            raise FileNotFoundError(
                f"Could not find one or more required asset files in `{asset_dir}`. "
                "Please ensure `labels.csv`, `metadata.csv`, and `split_coop.csv` are present."
            )

        labels_df = pd.read_csv(labels_fp)
        metadata_df = pd.read_csv(metadata_fp)
        class_names = metadata_df["class_name"].tolist()
        split_df = pd.read_csv(split_fp)

        data = datasets.DatasetDict()
        logger.info("Creating train/val/test splits for Flowers102...")
        for split in self.get_available_splits():
            split_filenames_df = split_df[split_df["split"] == split]

            merge_df = pd.merge(split_filenames_df, labels_df, on="filename")

            image_paths = [str(images_base_path / fname) for fname in merge_df["filename"].tolist()]
            labels = merge_df["class_idx"].tolist()

            data[split] = datasets.Dataset.from_dict({
                "image": image_paths,
                "label": labels
            }).cast_column("image", datasets.Image())

        for split in data:
            features = data[split].features
            features["label"] = datasets.ClassLabel(names=class_names)
            data[split] = data[split].cast(features)

        logger.info(f"Saving processed dataset to {output_path}")
        data.save_to_disk(str(output_path))

    def __getitem__(self, idx: int) -> Dict[str, Any]:
        sample = self.dataset[idx]
        label_id = sample["label"]
        label_name = self.dataset.features["label"].names[label_id]

        return {
            "image": sample["image"].convert("RGB"),
            "label_id": label_id,
            "label_name": label_name,
        }