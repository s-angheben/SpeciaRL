...
class BaseModelMerger(ABC):
    ...
    def save_lora_adapter(self, state_dict: dict[str, torch.Tensor]):
        ...
        # Try to copy original adapter config first
        original_config_path = os.path.join(self.config.local_dir, "lora_adapter", "adapter_config.json")
        if os.path.exists(original_config_path):
            print(f"Copying original adapter config from {original_config_path}")
            try:
                shutil.copy(original_config_path, os.path.join(lora_path, "adapter_config.json"))
            except Exception as e:
                print(f"Failed to copy config: {e}, falling back to generation")
                original_config_path = None
        else:
            original_config_path = None

        # Generate config if original not available
        if original_config_path is None:
            print("Generating adapter config from checkpoint parameters")
            ...
            if lora_key:
                # Extract one parameter to determine rank
                temp_param = state_dict[lora_params_names[0]]
                lora_rank = min(temp_param.shape[0], temp_param.shape[1])
                
                peft_dict = {
                    "r": lora_rank,
                    "lora_alpha": 0,  # Warning: lora_alpha defaults to 0, should be set manually
