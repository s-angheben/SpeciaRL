...
NOISE_PERCENTAGE = float(os.getenv("NOISE_PERCENTAGE", "0.0"))
if not 0.0 <= NOISE_PERCENTAGE <= 1.0:
    raise ValueError(f"NOISE_PERCENTAGE must be 0.0-1.0, got {NOISE_PERCENTAGE}")
...
def apply_classification_noise(classification: str) -> str:
    """Randomly upgrade or downgrade a classification category.

    Classification hierarchy (rank 1 to 5):
    SPECIFIC/MORE_SPECIFIC (rank 1) <-> LESS_SPECIFIC (rank 2) <-> GENERIC (rank 3) <-> ABSTAIN (rank 4) <-> WRONG (rank 5)

    - Upgrade = move toward rank 1
    - Downgrade = move toward rank 5
    - At boundaries: only one direction possible
    """
    categories = ["specific", "less specific", "generic", "abstain", "wrong"]
    # Note: more specific treated same as specific for noise purposes

    cat_lower = classification.lower()
    if cat_lower == "more specific":
        cat_lower = "specific"

    if cat_lower not in categories:
        return classification

    idx = categories.index(cat_lower)

    can_upgrade = idx > 0
    can_downgrade = idx < len(categories) - 1

    if can_upgrade and can_downgrade:
        direction = random.choice([-1, 1])  # -1=upgrade, +1=downgrade
    elif can_upgrade:
        direction = -1
    elif can_downgrade:
        direction = 1
    ...
    return categories[idx + direction]
...
def print_verification_table(parsed_items: List[Dict[str, Any]]) -> None:
    ...
    log_print(f"\n{'='*240}")
    log_print(f"{'STEP':>4} | {'IDX':>3} | {'SCORE':>5} | {'VERIFIER':^16} | {'NOISED':^8} | {'IS_BEST':^8} | {'GROUND TRUTH':<40} | {'PREDICTION':<40} | {'SAMPLE HASH':<64}")
    log_print(f"{'='*240}")
    ...
    for item in parsed_items:
        ...
        noised = "[NOISED]" if item.get("noised", False) else ""
        ...
        log_print(f"{batch_step:>4} | {idx:>3} | {score:>5.2f} | {verifier:^16} | {noised:^8} | {is_best:^8} | {gt_trunc:<40} | {pred_trunc:<40} | {sample_hash:<64}")
...
try:
    ...
    log_print(f"[INFO] Reward function (best prediction dedup noisy - mode: {REWARD_MODE}) initialized. API: {API_BASE_URL}")
    log_print(f"[INFO] Noise percentage: {NOISE_PERCENTAGE * 100:.1f}%")
...
def compute_score_batch(
    ...
    if NOISE_PERCENTAGE > 0:
        for item in parsed_items:
            if item["status"] == "VERIFIED" and random.random() < NOISE_PERCENTAGE:
                original = item["verifier_response"]
                noised = apply_classification_noise(original)
                if original != noised:
                    item["verifier_response"] = noised
                    item["noised"] = True
                    noised_count += 1
        if noised_count > 0:
            log_print(f"[NOISE] Applied noise to {noised_count} classifications (target: {NOISE_PERCENTAGE * 100:.1f}%)")
    ...
    log_print(f"[SUMMARY] Total Items: {len(parsed_items)} | Average Score: {average_score:.4f} | Best Predictions: {best_count} | Noised: {noised_count}")
